...
module.exports = AtomUtils = {
  ...
  extractES6StyleImportPath(moduleName, editorText = this.getEditor().getText()) {
  ...
  extractRequireStyleImportPath(moduleName, editorText = this.getEditor().getText()) {
    ...
    if (imports) {
        imports.forEach(importLine => {
          let moduleVariableDefinition = importLine.match('(.*)require', 'gi')

          if (moduleVariableDefinition && moduleVariableDefinition.length >= 2) {
            moduleVariableDefinition = moduleVariableDefinition[1];

            if (moduleVariableDefinition.indexOf(moduleName) > -1) {
              importPath = importLine.match('(?:.*?)require\\([\'"](.*?)[\'"]\\)');
  ...
  extractImportPath(moduleName) {
    ...
    let importPath = this.extractES6StyleImportPath(moduleName, editorText)
