...
module.exports = {
  ...
  extractImportPath(moduleName, useEmberPods) {
    ...
    return (
      ...
      this.extractEmberServicePath(moduleName, editorText, useEmberPods)
    );
  },
  ...
  extractEmberServicePath(moduleName, editorText, useEmberPods) {
    ...
    if (foundLine) {
      ...
      const podFile = npmPath.join(rootPath, 'app', 'services', dasherizedName);
      const nonPodFile = npmPath.join(
        rootPath,
        'app',
        dasherizedName,
        'service'
      );
      return useEmberPods ? podFile : nonPodFile;
    }
    ...
    if (!path && method) {
      ...
      if (fs.existsSync(path)) {
        ...
        if (!this.tmpPane) {
          ...
          this.tmpPane.addItem(pathItem, itemOptions);
