...
module.exports = {
   ...
   */
  onAtomConfigChange(callback) {
    return atom.config.onDidChange(callback);
  },
   ...
   * @return {TextEditor} Atom TextEditor instance
   */
  openEditor(path, options = {}) {
    const { usePendingPane, panePosition, openInSeparatePane } = options;
    const opts = {
      pending: typeof usePendingPane !== 'undefined' ? usePendingPane : true
    };

    if (openInSeparatePane) {
      opts.split = panePosition || 'right';
    }
  ...
  async openModule(module = {}, options = {}) {
    ...
    if (!modulePath && moduleMethod) {
      ...
      if (fs.existsSync(modulePath)) {
        const editor = await this.openEditor(modulePath, options);

        if (editor && moduleMethod) {
