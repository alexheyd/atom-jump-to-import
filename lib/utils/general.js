...
module.exports = {
  ...
  extractAllAliases(userOverrides, babelConfigs, projectConfigUrls) {
    ...
    const projectAliases = this.extractProjectAliases(
      projectConfigUrls,
      projectNames
    );
  ...
  convertToModule(cursorString, useEmberPods) {
    return PathUtils.getPathFromVariable(
      cursorString.replace(/'/g, ''),
      useEmberPods
    );
  },

  convertToHTMLBarsModule(rootPath, name, options) {
    const podPath = npmPath.join(
      rootPath,
      'app',
      'components',
      name,
  ...
  isHTMLBarsFile(textEditor) {
    const { scopeName } = textEditor.getGrammar();
    return scopeName === 'text.html.htmlbars';
  }
};
