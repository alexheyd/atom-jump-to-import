...
class JumpToImport {
  ...
  subscribeToConfigChanges() {
    this.subscriptions.add(atom.config.onDidChange('jump-to-import.disablePathOverrides', newValue => {
      this.options.disablePathOverrides = newValue;
    }));

    this.subscriptions.add(atom.config.onDidChange('jump-to-import.disableBabelRc', newValue => {
      this.options.disableBabelRc = newValue;
    }));

    this.subscriptions.add(atom.config.onDidChange('jump-to-import.pathOverrides', newValue => {
      this.options.pathOverrides = newValue;
    }));
  }
  ...
  applyMagicPaths(module) {
    ...
    if (module && magicPath) {
      ...
      // attempt to apply magic paths if module path is neither root absolute or relative
        ...
        console.log('path overrides disabled? ', this.options.disablePathOverrides);
