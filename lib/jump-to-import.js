...
class JumpToImport {
  ...
  observeConfigKey(key) {
    this.subscriptions.add(atom.config.onDidChange(`jump-to-import.${key}`, prop => {
  ...
  appendFileExtension(filePath) {
    ...
    const pathRelativeToParent = AtomUtils.isPathRelativeParent(filePath);
    const rootPath = pathRelativeToParent ? AtomUtils.getEditorDirPath() : AtomUtils.getEditorRootPath();
    let finalPath = null;
  ...
  applyMagicPaths(module) {
    ...
    if (module && magicPath) {
      ...
      module.path = this.appendFileExtension(realPath);
    }
