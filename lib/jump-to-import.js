...
class JumpToImport {
  ...
  openModule() {
    ...
    if (!modulePath && moduleMethod) {
      ...
      const pathRelativeToParent = AtomUtils.isPathRelativeParent(modulePath);
      const rootPath = pathRelativeToParent ? AtomUtils.getEditorDirPath() : AtomUtils.getEditorRootPath();
      const fullSystemPath = `${rootPath}/${modulePath}`;

      fs.exists(fullSystemPath, exists => {
        if (exists) {
          const openedEditor = this.openEditor(fullSystemPath);
