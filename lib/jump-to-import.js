...
class JumpToImport {
  ...
  packageSetup() {
    ...
    this.options = {
      ...
      disablePathOverrides: atom.config.get(
        'jump-to-import.disablePathOverrides'
      ),
  ...
  extractBabelConfig(babelConfig) {
    let config = null;
    const babelCfg = babelConfig && babelConfig.config
      ? babelConfig.config
      : null;
  ...
  getModule(cursorString) {
    ...
    let module = AtomUtils.isImportPath(cursorString)
      ? { path: modulePath }
      : pathFromVar;
  ...
  applyMagicPaths(module) {
    ...
    if (module && magicPath) {
      ...
      module.path = isNpmPath || hasExtension
        ? realPath
        : this.appendFileExtension(realPath);
    }
