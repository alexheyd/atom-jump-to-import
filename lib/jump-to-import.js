...
class JumpToImport {
  ...
  setup() {
    ...
    this.aliases = Utils.extractAllAliases(
      userOverrides,
      babelConfigs,
      projectConfigUrls
    );
  ...
  async goToModule(str) {
    ...
    if (isJavascript) {
      const cursorString = stringProvided
        ? str
        : AtomUtils.findStringUnderCursor();
      module = _getModule(cursorString, this.aliases[editorPath], options);
    } else if (isHTMLBars) {
      const cursorString = stringProvided
        ? str
        : AtomUtils.findHTMLBarsStringUnderCursor();
