...
class JumpToImport {
  ...
  extractBabelConfig(babelConfig) {
    let config = null;
    let babelCfg = babelConfig && babelConfig.config ? babelConfig.config : null;

    if (babelCfg) {
      let cfgPlugins = babelCfg.plugins;
  ...
  applyMagicPaths(module) {
    ...
    let editorPath = AtomUtils.getEditorDirPath();

    editorPath = editorPath.replace(rootPath + '/', '');
